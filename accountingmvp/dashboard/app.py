...
from src.parsers import ParserFactory, BankCSVParser, sanitize_csv_value
...
from src.models.enums import TransactionSource, MatchStatus
...
def render_summary_metrics(summary):
    ...
    with col1:
        st.metric(
            ...
            delta=f"{summary.match_rate:.0%} rate"
        )
        ...
        if matched:
            for m in matched[:50]:  # Limit display
                with st.container():
                    ...
                    with col1:
                        st.markdown(f"**Source:** {m.source_transaction.description[:40]}...")
                        st.caption(f"${m.source_transaction.amount} | {m.source_transaction.transaction_date}")
                    with col2:
                        st.markdown(f"**Target:** {m.target_transaction.description[:40]}...")
                        st.caption(f"${m.target_transaction.amount} | {m.target_transaction.transaction_date}")
                    with col3:
                        st.markdown(f"<span class='match-high'>{m.score.total_score:.0%}</span>", unsafe_allow_html=True)
                    st.divider()
        else:
        ...
        if review:
            for m in review[:50]:
                with st.container():
                    ...
                    with col1:
                        st.markdown(f"**Source:** {m.source_transaction.description[:30]}...")
                        st.caption(f"${m.source_transaction.amount}")
                    with col2:
                        st.markdown(f"**Target:** {m.target_transaction.description[:30]}...")
                        st.caption(f"${m.target_transaction.amount}")
                    with col3:
                        st.markdown(f"<span class='match-medium'>{m.score.total_score:.0%}</span>", unsafe_allow_html=True)
                    with col4:
                        if st.button("✅ Approve", key=f"approve_{m.source_transaction.id}"):
                            st.success("Approved!")
                        if st.button("❌ Reject", key=f"reject_{m.source_transaction.id}"):
                            st.warning("Rejected")
                    st.divider()
        else:
    ...
    with tab3:
        if unmatched:
            ...
            for m in unmatched[:20]:
                st.text(f"• {m.source_transaction.description[:50]} - ${m.source_transaction.amount}")
        else:
...
def main():
    ...
    if source_file and target_file:
        ...
        if st.button("🚀 Run Reconciliation", type="primary", use_container_width=True):
            with st.spinner("Processing files..."):
                try:
                    ...
                    st.success(f"✅ Parsed {len(source_txns)} source and {len(target_txns)} target transactions")
                    
                    if source_invalid or target_invalid:
                        st.warning(f"⚠️ {len(source_invalid)} source and {len(target_invalid)} target rows skipped due to validation errors")
                    
                    # Run reconciliation
                    engine = ReconciliationEngine(confidence_threshold=confidence_threshold)
        ...
        # Welcome message
        st.info("👈 Upload both source (bank statement) and target (payment records) files to begin reconciliation.")
